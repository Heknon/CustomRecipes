...
public class FileChecker {
    ...
    public FileChecker(CustomRecipes customRecipes, FileManager fileManager, String configurationFileName, boolean applyComments) {
        this.fileManager = fileManager;

        loadedFiles = new EnumMap<>(File.Type.class);

        java.io.File configFile = new java.io.File(customRecipes.getDataFolder(), configurationFileName);
        loadedFiles.put(File.Type.CREATED, new File(fileManager, configFile, YamlConfiguration.loadConfiguration(configFile)));

        if (applyComments) {
            loadedFiles.put(File.Type.RESOURCE, new File(null, null, YamlConfiguration.loadConfiguration(new InputStreamReader(fileManager.getConfigContent(new InputStreamReader(customRecipes.getResource(configurationFileName)))))));
        } else {
            loadedFiles.put(File.Type.RESOURCE, new File(null, null, YamlConfiguration.loadConfiguration(new InputStreamReader(customRecipes.getResource(configurationFileName)))));
        }
    }
